  ...
  return(
    <header className="flex flex-row justify-between items-center px-2 py-1 sm:px-6 md:px-8 md:py-2 border-b-2">
      ...
      <div className="hidden md:flex md:flex-row items-center gap-4">
        {
          state === false && (
            <Link href='/login/sign-in' onClick={handleSignIn} className="rounded-md dark:bg-white dark:text-black bg-black text-white md:py-2 md:px-3 dark:hover:bg-slate-500 hover:bg-slate-800 hidden md:block">
              Iniciar sesión
            </Link>
          ) 
        }
